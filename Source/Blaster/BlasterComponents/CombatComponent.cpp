...
{
	//UE_LOG(LogTemp, Display, TEXT("Granadas %d"),Grenades);
...
void UCombatComponent::SetAiming(bool bIsAiming)
{
	...
	if (Character->IsLocallyControlled() && EquippedWeapon->GetWeaponType() == EWeaponType::EWT_SniperRifle)
	{
		...
		{
			Character->GetMesh()->SetVisibility(!bIsAiming);
			//UE_LOG(LogTemp, Warning, TEXT("LLAMADO SET VISIBILITY %d"),bIsAiming);
		}
	}
...
{
	if (Character && Character->IsLocallyControlled() && bHoldingTheFlag)
	{
		Character->Crouch();
	}
}